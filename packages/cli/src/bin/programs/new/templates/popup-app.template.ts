export const popupAppTemplate = (): string => `import { useState } from 'react';

const HexaIcon = ({ size = 64, stroke = 'currentColor' }: { size?: number; stroke?: string }) => (
...
const GhostHexa = () => (
  <svg className="ghost-hexa" width="160" height="160" viewBox="0 0 100 100" fill="none">
...
export function App() {
  ...
  return (
    <div className={\`popup-root\${dark ? ' dark' : ''}\`}>
      <header className="popup-header">
        ...
        <div className="logo-icon">
          <HexaIcon size={64} stroke="currentColor" />
        </div>
        <h1>HexaJS</h1>
        <p className="tagline">Powerful JavaScript Framework Extension</p>
      ...
      <main className="popup-content">
        ...
        <div className="nav-cards">
          {[
            { icon: '📖', title: 'Documentation',  desc: 'Learn about HexaJS features and API', href: '#' },
            { icon: '</>',title: 'Code Examples',  desc: 'Browse sample projects and snippets',  href: '#' },
            { icon: '📄', title: 'Tutorials',       desc: 'Step-by-step guides for common tasks', href: '#' },
          ].map(({ icon, title, desc, href }) => (
            <a key={title} className="nav-card" href={href} onClick={e => e.preventDefault()}>
