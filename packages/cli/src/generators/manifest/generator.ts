    ...
    private applyUiEntries(manifest: ManifestV3): void {
        ...
        if (this.uiEntries.newtab) {
            ...
            if (this.resolved.platform === 'safari') {
                console.warn(`[ManifestGenerator] Safari requires user opt-in for new tab overrides. Users must enable it in Safari > Settings > General > "New tabs open with" and select this extension. With Profiles (macOS Sonoma+), this setting is per-profile.`);
            }
        } else {
